...
传统软件的开发过程遵循需求明确、逻辑确定的顺序：分析需求、设计架构、编码、测试、部署并维护。输入和代码确定后，输出具有可预测性，因此可以通过规定性测试验证功能和质量。然而，生成式人工智能系统并不完全遵循这种模式。它们的行为依赖于模型训练、数据质量以及概率采样，具有非确定性和适应性。一篇关于为[何生成式AI 功能不适用于传统
SDLC](https://www.linkedin.com/pulse/why-generative-ai-features-dont-fit-traditional-sdlc-stephen-higgins-grkye) 的文章指出，传统软件可以在需求阶段列出具体功能，而生成式AI 的“需求”更像假设，模型能力只能通过实验与数据迭代去探索；设计与实现则是反复试验和调参的过程，很难在项目开始时一次性确定方案。开发团队还需要处理数据收集、模型训练和评估等活动，模型的输出不再唯一，需要统计指标或人工评估来判断质量。此外，部署并非终点，模型在生产环境中会出现“模型漂移”并需要持续监测和再训练，这与传统软件维护阶段单纯修补代码不同。

本方法论综合了Google PAIR、Anthropic、OpenAI、Microsoft HAX等业界领先框架，以及2024-2025年最新学术研究，为生成式AI应用（LLM对话产品、AI Agent、RAG系统、传统应用AI增强、开发框架AI化等）提供系统性的需求分析与设计指导。其核心洞见是：**AI应用的需求与设计必须从"规格驱动"转向"目标驱动+评估驱动"的双轮模式**，在持续迭代中发现能力边界、校准用户期望、构建信任闭环。

---

## 一、需求分析

生成式AI 应用的需求不再是固定的功能列表，而是围绕业务目标和用户价值展开。
...
**目标D：用户提升和合作**
- D1 提升用户的认知水平
- D2 促进[共创](https://medium.com/design-ibm/design-principles-for-generative-ai-applications-791d00529d6f#:~:text=Principle%205%3A%20Design%20for%20Co)
